...
module EitherTest =
    ...
    let ``Right<TLeft, TRight>.Value should return its right value`` = test {
        let right = Either<exn, float>.Right(2.5).ToRight()
    ...
    let ``When Right<TLeft, TRight> then ToLeft should raise InvalidCastException`` = test {
    ...
    let ``Left<TLeft, TRight>.Value should return its left value`` = test {
        ...
        let left = Either<exn, float>.Left(err).ToLeft()
    ...
    let ``When Left<TLeft, TRight> then ToRight should raise InvalidCastException`` = test {
    ...
    let ``Either<TLeft, TRight> should have equality`` = test {
        let right1 = Either<exn, System.DateTime>.Right(System.DateTime(2015, 7, 12))
        let right2 = Either<exn, System.DateTime>.Right(System.DateTime(2015, 7, 12))
        let right3 = Either<exn, System.DateTime>.Right(System.DateTime(2015, 7, 13))
        let err = Exception("hoge")
        let left1 = Either<exn, System.DateTime>.Left(err)
        let left2 = Either<exn, System.DateTime>.Left(err)
        let left3 = Either<exn, System.DateTime>.Left(Exception("hoge"))
