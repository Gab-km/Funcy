...
module FuncyListNTTest =
    ...
    // Take
    let ``FuncyListNT.Take(cons, 3UL) returns a value of type Cons`` = test {
        let flist = FuncyList.Construct([| "my"; "name"; "is"; "FuncyList" |])
        let sut = flist.Take(3UL)
        do! assertEquals typeof<Cons<string>> <| sut.GetType()
    }
    let ``FuncyListNT.Take(nil, 3UL) returns a value of type Nil`` = test {
        let flist = FuncyList<string>.Construct([||])
        let sut = flist.Take(3UL)
        do! assertEquals typeof<Nil<string>> <| sut.GetType()
    }

    let ``FuncyListNT.Take(list, 3UL) takes first 3 elements`` = test {
        let flist = FuncyList.Construct([| "my"; "name"; "is"; "FuncyList" |])
        let sut = flist.Take(3UL)
        do! assertEquals sut <| FuncyList.Construct([| "my"; "name"; "is" |])
    }
    let ``FuncyListNT.Take(list, 3UL) returns list when the length of list is less than or equal to 3`` = test {
        let flist = FuncyList.Construct([| "FuncyList" |])
        let sut = flist.Take(3UL)
        do! assertEquals sut <| flist
    }
    let ``FuncyListNT.Take(list, 0UL) returns Nil`` = test {
        let flist = FuncyList.Construct([| "FuncyList" |])
        let sut = flist.Take(0UL)
    ...
    let ``FuncyListNT.Take commutes with Length function`` = test {
        ...
        do! assertEquals <|| (flist.Take(3UL).FMap(func), flist.FMap(func).Take(3UL))
    }
